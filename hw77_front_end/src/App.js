...
class App extends Component {
    ...
    render() {
        return (
            <div className="App">
                <div className="header">[Start a New Thread]</div>
                <form onSubmit={this.props.send}>
                    <label htmlFor="message">Message:</label>
                    <input name="message" type="text" required placeholder="Some message ..."
                ...
                <ol id="chatBox">
                    {this.props.error !== '' ? <div className="notification"
                                                    onClick={this.props.closeNotification}>{this.props.error}</div> : null}
                    {this.props.apiMessages.map((item, ndx) => (
                        <li key={item.id} className='li' id={ndx}>

                            <div className='thumbnail_div'>
                                <ImgThumbnail image={item.image}/>
                            </div>

                            {<div className="time_and_author_div">
                                <p>{moment(item.dateTime).format('llll')}</p>
                                <p>{item.author}</p>
                            </div>}

                            <div className="message_div">
                                <p className="post_number">Click on <button id={`Replied to post #${ndx} \n ${item.message}`} onClick={this.props.replyTo} className="reply_button">post
                                        #{ndx}</button> to reply
                                </p>
                                <p className="replied_message">{item.replyToMessage}</p>
                                <p>{item.message}</p>
                            </div>
...
const mapDispatchToProps = dispatch => {
    return {
        ...
        replyTo: (e) => dispatch(replyTo(e.currentTarget.id))
    };
};
